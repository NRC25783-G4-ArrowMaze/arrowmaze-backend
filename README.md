...
![Tests](https://img.shields.io/badge/tests-19%2F19%20passing-brightgreen?logo=jest)
...
API REST del juego de puzzles **Arrow Maze**: gestiona identidad de usuarios (registro, login, sesiones JWT), y — próximamente — distribución de niveles, progreso del jugador y leaderboards.
...
| F2 | API de distribución y actualización de definiciones de niveles | 📝 Spec lista — pendiente |
| F3 | API de recepción y consulta del progreso del jugador | 📝 Spec lista — pendiente |
| F4 | Leaderboard por nivel | ❌ Sin spec |
...
├── application/     # Casos de uso (RegisterAccount, Login, Logout) + puertos
│                    # (ITokenService, ICryptoService)
├── infrastructure/  # Adaptadores: Bcrypt, JWT, repositorios JSON/InMemory,
│                    # jobs (BlacklistCleanup con node-cron)
├── presentation/    # Express: AuthController, AuthRoutes, AuthMiddleware
...
> 📄 Documentación OpenAPI/Swagger: **pendiente** — se incorporará junto con F2/F3.
...
Jest + ts-jest, patrón **AAA** con nomenclatura `should_[resultado]_when_[condicion]`. Estado actual: **19/19 tests en 5 suites** ✅.

```
tests/
├── application/use-cases/       # RegisterAccount, Login, Logout (mocks de puertos)
├── presentation/middlewares/    # AuthMiddleware (escenarios 401/403 del Gherkin E2)
└── infrastructure/repositories/ # JsonSessionRepository (fs/promises mockeado)
```
